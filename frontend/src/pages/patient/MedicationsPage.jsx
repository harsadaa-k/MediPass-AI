  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        setRecords(data.records.filter((r) => r.record_type === 'medication'));
      } catch (err) {
        setError(err.detail || 'Could not load medications.');
      } finally {
        setLoading(false);
      }
    })();
  ...
  return (
    <div>
      <div className="page-header">
        <h1>Medications</h1>
        <p>All medication records from your verified timeline.</p>
      </div>
      ...
      {!loading && !error && records.length === 0 && (
        <div className="empty-state">No medication records found in your timeline yet.</div>
      )}
